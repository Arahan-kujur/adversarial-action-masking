...
We evaluate four victim algorithms: \textbf{Tabular Q-Learning} ($\varepsilon$-greedy, $\varepsilon = 0.15$, $\alpha = 0.1$), \textbf{Tabular PPO} (softmax policy, clipped surrogate loss, entropy bonus; details in Appendix~\ref{app:ppo}), \textbf{DQN} (2-layer MLP, experience replay, target network; used for Leduc), and \textbf{Tabular NFSP}~\citep{heinrich2016deep} (best-response Q-learning + average strategy, $\eta = 0.1$). All train via self-play: a single agent plays both roles with values indexed by player-specific information states.
...
\subsection{Neural-Scale Adversary (DQN Victim in Leduc)}

To verify the phenomenon persists with function approximation, we train a DQN victim in Leduc Poker against a neural adversary (2-layer MLP, 32 hidden units, REINFORCE outer loop):

\begin{center}
\begin{tabular}{lc}
\toprule
Strategy & Leduc DQN Victim Reward \\
\midrule
None & $-0.09 \pm 0.11$ \\
Random ($p=0.5$) & $-0.74 \pm 0.17$ \\
Fixed (raise) & $-0.35 \pm 0.31$ \\
Neural adversary & $\mathbf{-3.29 \pm 0.48}$ \\
\bottomrule
\end{tabular}
\end{center}

The neural adversary is $4.5\times$ more damaging than random masking and $9\times$ worse than fixed removal, driving the DQN victim to $-3.29$ per hand. This confirms that adversarial action removal scales to function approximation agents in richer games. The bi-level REINFORCE training successfully learns a targeted masking strategy that exploits the victim's learned policy structure.
