"""Leduc-10 scale experiment: 10 ranks x 2 suits = 20 cards, 1500+ info states.

DQN victim + neural adversary. This is the "large" benchmark.
"""
import sys, os
...
class NeuralAdversary(nn.Module):
    def __init__(self, input_dim, num_actions):
        ...
        self._encoder, _ = get_encoder("leduc10")
...
def make_victim():
    return DQNAgent(game="leduc10", num_actions=NUM_ACTIONS, lr=1e-3,
                    epsilon_start=1.0, epsilon_end=0.05, epsilon_decay=0.9999,
                    buffer_size=30000, batch_size=64, target_update_freq=500)
...
def count_states(env, agent, rng, n=20000):
    s = set()
    for _ in range(n):
        env.reset(rng=rng)
...
def main():
    print("="*65, flush=True)
    print("  LEDUC-10: 10 ranks x 2 suits = 20 cards", flush=True)
    print("="*65, flush=True)

    rng = np.random.default_rng(0)
    env = MaskedLeducNPoker(LeducNPokerEnv(num_ranks=10))
    v = make_victim(); pretrain(env, v, rng, 5000)
    states = count_states(env, v, rng, 30000)
    ...
    for seed in SEEDS:
        ...
        base = MaskedLeducNPoker(LeducNPokerEnv(num_ranks=10))
        victim = make_victim()
        ...
        r_rand = evaluate_agent(MaskedLeducNPoker(LeducNPokerEnv(num_ranks=10)),
        ...
        r_fixed = evaluate_agent(MaskedLeducNPoker(LeducNPokerEnv(num_ranks=10)),
        ...
        ne = MaskedLeducNPoker(LeducNPokerEnv(num_ranks=10))
        vn = make_victim(); pretrain(ne, vn, rng, PRETRAIN)
        _, dim = get_encoder("leduc10")
    ...
    r_min, r_max = -21.0, 21.0
    print(f"\n{'='*65}\n  LEDUC-10 RESULTS ({len(SEEDS)} seeds)\n{'='*65}", flush=True)
    for s in results:
